...
class Agent(object):
    ...
    def request(self, method, uri, headers=None, bodyProducer=None):
        ...
        if self.persistent:
            ...
            def execute(_ignored):
                return self._request(method, scheme, host, port, path, headers,
                    bodyProducer, sem.release)
    ...
    def _request(self, method, scheme, host, port, path, headers, bodyProducer,
        ...
        for p in protos:
            if p.state == 'CONNECTION_LOST':
                protos.remove(p)
            elif p.state == 'QUIESCENT' and not maybeDisconnected:
                d = defer.succeed(p)
                maybeDisconnected = True
                
        if not maybeDisconnected:
            # new connection
            d = self._connect(scheme, host, port)
        req = Request(method, path, headers, bodyProducer,
                      persistent=self.persistent)

        def saveProtocol(proto):
            if self.persistent and proto not in protos:
                protos.append(proto)
            return proto
        d.addCallback(saveProtocol)
                ...
                if f.check(ResponseFailed):
                    for reason in f.value.reasons:
                        if (isinstance(reason, failure.Failure) and 
                            ...
                            def retry(proto):
                                proto.quiescentCallback = quiescentCallback
                                d = proto.request(req)
                                return d
                            d = self._connect(scheme, host, port)
                            d.addCallback(saveProtocol)
